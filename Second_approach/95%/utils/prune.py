...
def prune_weights(net):
    """This function prunes the 95% of the lowest weights in each layer of the network and sets them to zero"""
...
def apply_mask(net, individuo):
    ...
    for (param_net, param_individuo) in zip(net.parameters(), individuo.parameters()):
        ...
        # Calcula el percentil 95 para determinar el umbral
        percent = torch.quantile(pesos_individuo.abs().flatten(), 0.95)
        
        # Crea la máscara: 1 donde el valor >= percentil_95, 0 donde < percentil_95
