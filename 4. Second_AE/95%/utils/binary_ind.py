...
def make_to_binary(individuo):
    ...
    for name, param in individuo.named_parameters():
        if "weight" in name or "bias" in name:
            # Calcula el percentil 5 directamente sobre los valores de los parámetros
            threshold = param.quantile(0.95)

            # Binariza: 1 para valores mayores al percentil 5, 0 para los demás
...
def modify_weights(network):
    """
    Modifica la red asegurando que los pesos binarizados siempre mantengan un 5% de activación.
    ...
    with torch.no_grad():
        ...
        # Total de pesos y objetivo del 5%
        total_weights = weights.numel()
        target_ones = int(0.05 * total_weights)  # Queremos exactamente el 5%
        ...
        # Modificar aleatoriamente un 1% de los pesos
        num_to_flip_1_to_0 = min(len(ones_indices), int(0.01 * total_weights))
        num_to_flip_0_to_1 = min(len(zeros_indices), int(0.01 * total_weights))
        ...
        # Ajustar para garantizar exactamente el 5% de activación
        ones_indices = (weights == 1).nonzero(as_tuple=True)[0]
        zeros_indices = (weights == 0).nonzero(as_tuple=True)[0]
